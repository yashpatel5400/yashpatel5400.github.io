...
\begin{figure*}
  ...
  \caption{CPO leverages informative, non-convex conformal prediction regions for robust predict-then-optimize decision making. CPO uses a score function such that the resulting prediction regions can be decomposed into convex subregions over which optimization can be carried out efficiently. Visual summaries $\{\xi^{(i)}\}$ of the prediction region can similarly be efficiently sampled to gain intuition on the optimal decision $w^{*}$.
  }
\end{figure*}
...
\end{equation}
We refer to this score as ``Generalized Probabilistic Conformal Prediction,'' (GPCP) whose validity follows from that of the original PCP framework \citep{wang2022probabilistic}.
...
\begin{algorithm}
  \caption{\label{alg:VolumeEst} VolumeEst}
  \begin{algorithmic}[1]
    \Procedure{VolumeEst}{}
    ...
    \State \textbf{Return} $|B_{\widehat{q}}| \sum_{k=1}^{K} \frac{1}{M}\sum_{m=1}^{M}\mathbb{I}\left[c_{k, m} \in V(\widehat{c}_{k})\right]$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}

\begin{algorithm}
  \caption{\label{alg:CPO} CPO}
    \Procedure{CPO}{}
    ...
    \State $\widehat{\ell}_K \gets \frac{1}{|\mathcal{D}_{\mathcal{C}_2}|}\sum_{i=1}^{|\mathcal{D}_{\mathcal{C}_2}|}\text{VolumeEst}(x^{(i)}, q, \widehat{q}_K)$
    \EndFor
    \State $K^{*} \gets \argmin_{K} \left|\widehat{\ell}_K - \widehat{\ell}_{K + 1}\right| \leq \epsilon$
    \State \textbf{Return} $\text{CPO-Opt}(x, q, T, K^{*}, \widehat{q}_{K^{*}})$
    \EndProcedure
    \end{algorithmic}
\end{algorithm}
